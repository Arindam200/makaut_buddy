...
const Card = React.memo(({ id , sem , subject , type , title, description , link }) => {
  ...
  const [showConfirmationWindow , setShowConfirmationWindow] = useState(false);
    ...
    useEffect(() => {
        ...
        descriptionRef.current ? descriptionRef.current.value = description : null ;
        linkRef.current ? linkRef.current.value = link : null ;
  
    } , [openEditWindow]);
    
    function updateResource(data){
      axios.put(`https://makaut-buddy-back-end.iamsagar762.workers.dev/updateResource/${id}` , 
    ...
    function deleteResource(id){
      axios.delete(`https://makaut-buddy-back-end.iamsagar762.workers.dev/deleteResource/${id}` ,
          {
    ...
    <div className="flex flex-col border-2 border-[#FFFFFF20] rounded-lg m-1">
      {
        ...
        <section className="w-[50%] h-[90%] bg-white rounded-md z-30 ">
                <section className="flex items-center justify-center w-[100%] bg-white text-black ">
                    <section className="p-[10px] font-semibold" >Edit Resource</section>
                </section>
                            ...
                            <section className="flex flex-row gap-4">
                                  ...
                                  <button className="bg-[#121212] p-[10px] text-[#e3e4ea] min-w-[200px] max-w-[300px] rounded-md"
                                    ...
                                    onClick={() =>{
                                      ...
                                      if(semRef.current.value && semRef.current.value !== sem){
                                        updateData["sem"] = semRef.current.value.trim();
                                      }else if(subjectRef.current.value && subjectRef.current.value.trim() !== subject){
                                        updateData["subject"] = subjectRef.current.value.trim();
                                      }else if(headingRef.current.value && headingRef.current.value.trim() !== title){
                                        updateData["heading"] = headingRef.current.value.trim();
                                      }else if(descriptionRef.current.value && descriptionRef.current.value.trim() !== description){
                                        updateData["description"] = descriptionRef.current.value.trim();
                                      }else if(linkRef.current.value && linkRef.current.value.trim() !== link){
        ...
        <section className="h-[70%] w-[60%] absolute z-20 top-[30%] left-[35%]">
            <section className="h-[35%] w-[50%] bg-white text-black ">
                ...
                <footer className="h-[30%] flex items-center justify-end gap-3">
                   <button className="p-[10px] bg-[#D7DBE4] rounded-md "
                      onClick={() => {
                          setShowConfirmationWindow(false);
                      }}
                   >Cancel</button>
                   <button className="p-[10px] bg-[#F70000] rounded-md text-white mr-[20px]"
                    onClick={() => {
                      deleteResource(id)
                    }}
                   >Delete</button>
